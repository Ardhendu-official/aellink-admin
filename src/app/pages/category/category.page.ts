...
export class CategoryPage implements OnInit {
  ...
  displayedColumns: string[] = ['category_id', 'category_image', 'category_name', 'category_description', 'category_status', 'actions'];
  dataSource;
  ...
  fetchGroceryCategory(category_id){
    this.loading.presentLoading()
    this.http.get(`${this.apiservices.api_url}parentcategory/${category_id}`).pipe(finalize(() => this.loading.dismissLoading()))
    .subscribe((res)=>this.dataSource.data = res as CategoryDatas[],(reserror)=>{
      console.log(reserror.error.detail);
